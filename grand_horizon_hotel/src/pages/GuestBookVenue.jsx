...
const GuestBookVenue = () => {
  ...
  useEffect(() => {
    API.get('/venues/', { headers }).then(res => {
      const data = Array.isArray(res.data) ? res.data : [];
      setVenue(data.find(v => v.id === parseInt(venueId)));
    }).catch(() => navigate('/guest/venues'));
  }, [venueId]);

  const basePrice = venue ? parseFloat(venue.price_per_day) : 0;

  // Parse packages based on event type
  const getPackages = () => {
    if (!venue?.additional_packages) return [];
    const sections = venue.additional_packages.split('|');
    for (const section of sections) {
      if (section.trim().toLowerCase().startsWith(eventType.toLowerCase())) {
        const parts = section.split(' ').slice(1).join(' ');
        return parts.split(',').map(p => p.trim()).filter(p => p.includes(':'));
      }
    }
    return [];
  };
  ...
  const handleBook = async () => {
    if (!date) return showPopup('Select a date.');
    try {
      const res = await API.post('/venues/book/', { venue_id: venueId, event_type: eventType, date, guests, selected_packages: selectedPackages }, { headers });
      setBooking(res.data.booking); setStep(2);
  ...
  const handlePay = async () => {
    ...
    try {
      const res = await API.post('/mpesa/pay/', { phone_number: phone, amount: totalPrice, booking_id: `VEN-${booking.id}` });
      setPaymentStatus({ checkout_id: res.data.checkout_request_id, status: 'pending' }); setStep(3);
  ...
  if (!venue) return <div className="min-h-screen bg-stone-50"><GuestNavbar /><div className="flex justify-center py-40"><div className="animate-spin w-10 h-10 border-4 border-amber-700 border-t-transparent rounded-full" /></div></div>;
    ...
    <div className="min-h-screen bg-stone-50 font-sans">
      ...
      <section className="relative bg-stone-900 text-white py-10 px-8 overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto">
          <button onClick={() => navigate('/guest/venues')} className="flex items-center gap-2 text-amber-300 hover:text-amber-200 mb-3"><ArrowLeft className="w-4 h-4" /> Back</button>
          <h1 className="text-3xl font-serif font-bold">Book {venue.name}</h1>
        </div>
      </section>
      ...
      <div className="max-w-xl mx-auto px-8 -mt-6 relative z-10 pb-16">
        {step === 1 && (
          <div className="bg-white rounded-2xl shadow-xl border p-8">
            <div className="flex items-center gap-4 mb-6">
              <div className="bg-amber-100 w-12 h-12 rounded-xl flex items-center justify-center"><PartyPopper className="w-6 h-6 text-amber-700" /></div>
              <div><h2 className="text-lg font-bold">{venue.name}</h2><p className="text-stone-500 text-sm">{venue.capacity} guests • KES {parseFloat(venue.price_per_day).toLocaleString()}/day</p></div>
            </div>
            <div className="space-y-4 mb-4">
              <div><label className="block text-xs font-medium text-stone-500 mb-1">Event Type</label><select value={eventType} onChange={e => { setEventType(e.target.value); setSelectedPackages([]); }} className="w-full px-4 py-3 border rounded-xl outline-none bg-white"><option value="wedding">Wedding</option><option value="birthday">Birthday</option><option value="other">Other</option></select></div>
              ...
              <div><label className="block text-xs font-medium text-stone-500 mb-1">Guests</label><input type="number" value={guests} min={1} max={venue.capacity} onChange={e => setGuests(Math.max(1, Math.min(venue.capacity, parseInt(e.target.value) || 1)))} className="w-24 px-4 py-3 border rounded-xl outline-none" /></div>
            </div>
            {packageList.length > 0 && (
              <div className="mb-4">
                <label className="block text-xs font-medium text-stone-500 mb-2">Packages for {eventType}</label>
                <div className="flex flex-wrap gap-2">
                  {packageList.map(pkg => { const [name, price] = pkg.split(':'); return (
                    <button key={name} onClick={() => togglePackage(name.trim())} className={`px-4 py-2 rounded-full text-xs font-medium transition ${selectedPackages.includes(name.trim()) ? 'bg-amber-700 text-white' : 'bg-stone-100 text-stone-600 hover:bg-amber-100'}`}>{name.trim()} (+KES {price})</button>
                  );})}
                </div>
              </div>
            )}
            {date && (
              <div className="bg-amber-50 rounded-xl p-4 mb-6">
                <div className="flex justify-between text-sm mb-1"><span>{date}</span><span><Users className="w-3 h-3 inline mr-1" />{guests} guests</span></div>
            ...
            <button onClick={handleBook} disabled={!date} className="w-full bg-amber-700 text-white py-3 rounded-xl font-semibold hover:bg-amber-800 disabled:opacity-50">Confirm • KES {totalPrice.toLocaleString()}</button>
          </div>
        )}
        ...
        {step === 2 && booking && (
          <div className="bg-white rounded-2xl shadow-xl border p-6">
            ...
            <div className="bg-amber-50 rounded-lg p-3 mb-4"><div className="flex justify-between text-sm"><span>Booking #{booking.id}</span><span>{venue.name}</span></div><div className="text-xs text-stone-400 mb-1">{date} • {guests} guests</div><div className="border-t border-amber-200 pt-2"><span className="font-bold text-amber-800">KES {totalPrice.toLocaleString()}</span></div></div>
