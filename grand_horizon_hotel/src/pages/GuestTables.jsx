...
import { UtensilsCrossed, Users, MapPin, Search, Grid3X3, List } from 'lucide-react';
...
const GuestTables = () => {
  ...
  useEffect(() => { fetchTables(searchDate); }, [searchDate, fetchTables]);
  ...
  const handleReserve = (tableId) => {
    if (!isLoggedIn) {
      ...
      navigate(`/guest/tables/book/${tableId}?date=${searchDate}`);
    }
  };

  return (
    <div className="min-h-screen bg-stone-50">
      {/* Show GuestNavbar if logged in, otherwise show public Navbar */}
      ...
      <div className="max-w-7xl mx-auto px-8 py-10">
        <div className="bg-white rounded-2xl border p-5 -mt-16 relative z-10 shadow-lg mb-10">
          <div className="flex flex-wrap items-center gap-4">
            <div className="flex items-center gap-3">
              ...
              <button onClick={()=>fetchTables(searchDate)} className="bg-amber-700 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-amber-800 flex items-center gap-1"><Search className="w-4 h-4" /> Search</button>
            </div>
        ...
        {loading ? <div className="text-center py-16"><div className="animate-spin w-10 h-10 border-4 border-amber-700 border-t-transparent rounded-full mx-auto" /></div> : tables.length===0 ? (
          <div className="text-center py-16 bg-white rounded-2xl border"><UtensilsCrossed className="w-16 h-16 text-stone-200 mx-auto mb-4" /><p className="text-stone-500">No tables for {searchDate}.</p></div>
        ) : (
          <>
            {viewMode==='grid' ? (
                ...
                {paginated.map(table=>(
                  <div key={table.id} className="group bg-white rounded-2xl border overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                    <div className="h-56 bg-stone-100 relative overflow-hidden">
                      {table.image?<img src={table.image} loading="lazy" className="w-full h-full object-cover group-hover:scale-105 transition duration-700"/>:<div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-amber-50 to-orange-100"><UtensilsCrossed className="w-16 h-16 text-amber-300"/></div>}
                      <div className="absolute inset-0 bg-gradient-to-t from-black/40 to-transparent"/>
                      <div className="absolute bottom-4 left-4 right-4 flex justify-between items-end">
                        <span className="text-white font-bold text-lg">Table {table.table_number}</span>
                        <span className={`px-3 py-1 rounded-full text-xs font-medium ${table.available?'bg-amber-500 text-white':'bg-red-500 text-white'}`}>
                    ...
                    <div className="p-5">
                      ...
                      <div className="flex justify-between items-center pt-4 border-t">
                        ...
                        {table.available?(
                          <button onClick={()=>handleReserve(table.id)} className="bg-amber-700 text-white px-6 py-2.5 rounded-full text-sm font-medium hover:bg-amber-800 shadow-md">
                            Reserve
                          </button>
                        ):(
              ...
              <div className="space-y-3">
                {paginated.map(table=>(
                  <div key={table.id} className="bg-white rounded-2xl border p-5 flex items-center justify-between hover:shadow-md transition">
                    <div className="flex items-center gap-4">
                      <div className="w-16 h-16 rounded-xl bg-stone-100 overflow-hidden flex-shrink-0">
                        {table.image?<img src={table.image} className="w-full h-full object-cover"/>:<div className="w-full h-full flex items-center justify-center"><UtensilsCrossed className="w-8 h-8 text-stone-300"/></div>}
                      </div>
                      <div>
                        <h3 className="font-bold text-stone-800">
                          Table {table.table_number}{' '}
                          <span className={`text-xs px-2 py-0.5 rounded-full ml-2 ${table.available?'bg-amber-100 text-amber-700':'bg-red-100 text-red-600'}`}>
                            {table.available?`${table.capacity} pax`:'Booked'}
                          </span>
                        </h3>
                        <p className="text-stone-500 text-sm">{table.location||'Main Dining'} • <Users className="w-3 h-3 inline"/> {table.capacity} guests</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-6">
                      <span className="font-bold text-stone-800">KES {parseFloat(table.price_per_slot).toLocaleString()}</span>
                      {table.available?(
                        <button onClick={()=>handleReserve(table.id)} className="bg-amber-700 text-white px-5 py-2 rounded-full text-sm font-medium hover:bg-amber-800">
                          Reserve
                        </button>
                      ):(
