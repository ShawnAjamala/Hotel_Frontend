...
import { ArrowLeft, BedDouble, UtensilsCrossed, Presentation, PartyPopper, CheckCircle, LogOut, Trash2, Filter, X } from 'lucide-react';
...
const StaffBookings = () => {
  ...
  const handleDelete = (booking) => {
    showPopup('danger', `Permanently delete this ${typeLabels[booking.type]} booking? This action cannot be undone.`, async () => {
    ...
    <div className="min-h-screen bg-stone-50 font-sans">
      ...
      {popup.show && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          {/* Popup Card */}
          <div className="relative bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full mx-4 animate-in">
            <div className="text-center">
              ...
              <h3 className="text-lg font-bold text-stone-800 mb-2">
                {popup.type === 'danger' ? 'Are you sure?' : 'Confirm Action'}
              </h3>
              ...
              <div className="flex gap-3">
                <button
                  onClick={closePopup}
                  className="flex-1 px-4 py-2.5 border border-stone-200 rounded-xl text-stone-600 font-medium hover:bg-stone-50 transition"
                >
                  Cancel
                </button>
                <button
                ...
                >
                  {popup.type === 'danger' ? 'Delete' : 'Confirm'}
                </button>
              </div>
            </div>
          </div>
      ...
      <div className="bg-amber-800 text-white">
        <div className="max-w-7xl mx-auto px-8 py-10">
          <button onClick={() => navigate('/staff/dashboard')} className="flex items-center gap-2 text-amber-200 hover:text-white transition mb-3">
            <ArrowLeft className="w-4 h-4" /> Back to Dashboard
          </button>
          <h1 className="text-3xl font-bold">All Bookings</h1>
          <p className="text-amber-100 mt-1">{bookings.length} total booking{bookings.length !== 1 ? 's' : ''}</p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-8 py-8">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex gap-1.5">
                {['all', 'room', 'table', 'conference', 'venue'].map(opt => (
                  <button key={opt} onClick={() => setFilter(opt)} className={`px-3 py-1.5 rounded-full text-xs font-medium transition ${filter === opt ? 'bg-amber-700 text-white' : 'bg-stone-100 text-stone-600 hover:bg-amber-100'}`}>
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex gap-1.5">
                {['all', 'pending', 'confirmed', 'checked_in', 'checked_out', 'cancelled'].map(opt => (
                  <button key={opt} onClick={() => setStatusFilter(opt)} className={`px-3 py-1.5 rounded-full text-xs font-medium transition ${statusFilter === opt ? 'bg-amber-700 text-white' : 'bg-stone-100 text-stone-600 hover:bg-amber-100'}`}>
        ...
        {loading ? (
          ...
          <div className="text-center py-16 bg-white rounded-2xl border border-stone-200"><p className="text-stone-400">No bookings found.</p></div>
        ) : (
