...
import { Hotel, Calendar, User, LogOut, BedDouble, UtensilsCrossed, Presentation, PartyPopper, Menu, X, LayoutDashboard } from 'lucide-react';
    ...
    <>
      <nav className="flex items-center justify-between px-4 sm:px-8 py-4 bg-white/80 backdrop-blur-md sticky top-0 z-40 border-b border-stone-100">
        ...
        <div className="hidden md:flex items-center gap-6">
          <button onClick={() => navigate('/staff/dashboard')} className="flex items-center gap-1.5 text-stone-500 hover:text-amber-700 text-sm transition">
            <Hotel className="w-4 h-4" /> Home
          </button>
          <button onClick={() => navigate('/staff/rooms/create')} className="flex items-center gap-1.5 text-stone-500 hover:text-amber-700 text-sm transition">
            <BedDouble className="w-4 h-4" /> Rooms
          </button>
          <button onClick={() => navigate('/staff/tables/create')} className="flex items-center gap-1.5 text-stone-500 hover:text-amber-700 text-sm transition">
            <UtensilsCrossed className="w-4 h-4" /> Tables
          </button>
          <button onClick={() => navigate('/staff/conference/create')} className="flex items-center gap-1.5 text-stone-500 hover:text-amber-700 text-sm transition">
            <Presentation className="w-4 h-4" /> Conference
          </button>
          <button onClick={() => navigate('/staff/venues/create')} className="flex items-center gap-1.5 text-stone-500 hover:text-amber-700 text-sm transition">
            <PartyPopper className="w-4 h-4" /> Venues
          </button>
          <button onClick={() => navigate('/staff/bookings')} className="flex items-center gap-1.5 text-amber-700 font-medium text-sm transition">
        ...
        <div className="flex md:hidden items-center gap-3">
          <button onClick={() => navigate('/staff/profile')} className="bg-amber-100 w-9 h-9 rounded-full flex items-center justify-center hover:bg-amber-200 transition">
