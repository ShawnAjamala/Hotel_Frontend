...
const GuestBookConference = () => {
  ...
  useEffect(() => {
    ...
    API.get('/conference/', { headers })
      .then(r => {
        ...
        const found = data.find(x => x.id === parseInt(roomId));
  ...
  const hrs = st && et
    ? Math.max(0, Math.floor((new Date(`2000-01-01T${et}`) - new Date(`2000-01-01T${st}`)) / 3600000))
    : 0;
  const base = room ? hrs * parseFloat(room.price_per_hour) : 0;
  const list = room?.additional_packages
    ? room.additional_packages.split(',').map(p => p.trim()).filter(p => p.includes(':'))
    : [];
  const extra = pkg.reduce((s, p) => {
    const f = list.find(x => x.startsWith(p));
    return f ? s + parseFloat(f.split(':')[1]) : s;
  }, 0);
  ...
  const book = async () => {
    if (!date || !st || !et) return showPopup('Please select a date and time.');
    if (hrs <= 0) return showPopup('End time must be after start time.');
    try {
      const r = await API.post('/conference/book/', {
        room_id: roomId, date, start_time: st, end_time: et,
        guests, selected_packages: pkg,
      }, { headers });
  ...
  const pay = async () => {
    if (!phone) return showPopup('Please enter your M-Pesa number.');
    const p = phone.startsWith('254') ? phone : `254${phone.replace(/^0+/, '')}`;
    try {
      const r = await API.post('/mpesa/pay/', {
        phone_number: p, amount: total, booking_id: `CONF-${booking.id}`,
      });
    ...
    <div className="min-h-screen bg-stone-50 font-sans">
      ...
      <section className="relative bg-stone-900 text-white py-12 px-6 overflow-hidden">
        ...
        <div className="relative max-w-2xl mx-auto">
          <button
            onClick={() => navigate('/guest/conference')}
          ...
          <div className="flex items-center gap-4 mt-2">
            <div className="w-16 h-16 rounded-xl overflow-hidden bg-white/10 flex-shrink-0">
              {room.image
                ? <img src={room.image} loading="lazy" alt={room.name} className="w-full h-full object-cover" />
                : <div className="w-full h-full flex items-center justify-center"><Presentation className="w-8 h-8 text-white/50" /></div>
              }
            </div>
            <div className="min-w-0">
              <h1 className="text-2xl sm:text-3xl font-serif font-bold truncate">{room.name}</h1>
              <p className="text-stone-300 text-sm mt-1">
                {room.capacity} pax &bull; KES {parseFloat(room.price_per_hour).toLocaleString()}/hr
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8 pb-16">
        ...
        {step === 1 && (
          <div className="bg-white rounded-2xl shadow-xl border p-6 sm:p-8">
            <h2 className="text-xl font-bold text-stone-800 mb-6">Book Conference Room</h2>
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  <input
                    ...
                    min={1}
                    max={room.capacity}
                    onChange={e => setGuests(Math.max(1, Math.min(room.capacity, parseInt(e.target.value) || 1)))}
                  ...
                  />
                  <span className="text-stone-400 text-sm">Max: {room.capacity}</span>
                </div>
              </div>
            </div>
            ...
            {list.length > 0 && (
              <div className="mt-5">
                <label className="block text-sm font-medium text-stone-700 mb-2">Additional Packages</label>
            ...
            {/* Cost summary — only shown when time is selected */}
            {date && st && et && hrs > 0 && (
              <div className="mt-5 bg-amber-50 rounded-2xl p-4 border border-amber-100">
                <div className="flex items-center justify-between text-sm text-stone-500 mb-2">
                  <span className="flex items-center gap-1">
                    <Clock className="w-4 h-4 text-amber-600" />{hrs}h session
                  </span>
                  <span className="flex items-center gap-1">
                    <Users className="w-4 h-4 text-amber-600" />{guests} guests
                  </span>
                </div>
                <p className="text-sm text-stone-400 mb-2">{date} &bull; {st} – {et}</p>
                {extra > 0 && (
            ...
            >
              {hrs > 0 ? `Confirm Booking · KES ${total.toLocaleString()}` : 'Select Date & Time to Continue'}
            </button>
          </div>
        )}
          ...
          <div className="bg-white rounded-2xl shadow-xl border p-6 sm:p-8">
            ...
            <div className="bg-amber-50 rounded-2xl p-4 border border-amber-100 mb-5">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="bg-amber-200 text-amber-800 px-3 py-1 rounded-full text-xs font-medium">{room.name}</span>
              </div>
            ...
            <button
              onClick={() => setStep(1)}
              className="w-full text-stone-400 py-3 mt-2 hover:text-stone-600 text-sm font-medium transition-colors"
            >
            ...
            {payStatus?.status === 'completed' ? (
              <>
                ...
                <button
                  ...
                  className="bg-amber-700 text-white px-8 py-3 rounded-full font-medium hover:bg-amber-800 shadow-lg transition-colors"
                >
                  View My Bookings
                </button>
              </>
