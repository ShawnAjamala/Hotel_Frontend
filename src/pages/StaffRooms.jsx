...
import { Plus, Edit, Trash2, BedDouble, X, ImagePlus, Users, Building, Grid3X3, List } from 'lucide-react';
...
const StaffRooms = () => {
  ...
  const [form, setForm] = useState({ room_number: '', room_type: 'single', price_per_night: '', max_guests: '', description: '', amenities: '' });
  ...
  const fetchRooms = async () => { setLoading(true); try { const res = await API.get('/rooms/', { headers }); setRooms(Array.isArray(res.data) ? res.data : (res.data.rooms || [])); } catch (err) {} setLoading(false); };
  const handleImageChange = (e) => { const file = e.target.files[0]; if (file) { setImage(file); setImagePreview(URL.createObjectURL(file)); } };
  const handleCreate = async (e) => { e.preventDefault(); const fd = new FormData(); Object.entries(form).forEach(([k,v]) => fd.append(k,v)); if(image) fd.append('image',image); try { await API.post('/rooms/create/', fd, { headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'multipart/form-data' } }); resetForm(); fetchRooms(); } catch (err) {} };
  const handleUpdate = async (e) => { e.preventDefault(); const fd = new FormData(); Object.entries(form).forEach(([k,v]) => fd.append(k,v)); if(image) fd.append('image',image); try { await API.put(`/rooms/${editingRoom.id}/update/`, fd, { headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'multipart/form-data' } }); resetForm(); fetchRooms(); } catch (err) {} };
  const handleDelete = async (id) => { if (!window.confirm('Deactivate?')) return; try { await API.delete(`/rooms/${id}/delete/`, { headers }); fetchRooms(); } catch (err) {} };
  const openEdit = (room) => { setEditingRoom(room); setForm({ room_number: room.room_number, room_type: room.room_type, price_per_night: parseFloat(room.price_per_night)||'', max_guests: room.max_guests, description: room.description||'', amenities: room.amenities||'' }); setImagePreview(room.image||null); setImage(null); setShowForm(true); };
            ...
            {viewMode==='grid'?(
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {paginatedRooms.map(room=>(
                  <div key={room.id} className="bg-white rounded-2xl border overflow-hidden hover:shadow-lg transition group"><div className="h-48 bg-stone-100 relative">{room.image?<img src={room.image} className="w-full h-full object-cover group-hover:scale-105 transition duration-500" />:<div className="w-full h-full flex items-center justify-center"><BedDouble className="w-12 h-12 text-stone-300" /></div>}<div className="absolute top-3 left-3 bg-white/90 px-3 py-1 rounded-full text-sm font-bold">#{room.room_number}</div><div className="absolute top-3 right-3 bg-amber-700 text-white px-3 py-1 rounded-full text-xs capitalize">{room.room_type}</div></div><div className="p-5"><div className="flex justify-between mb-2"><h3 className="font-bold">Room {room.room_number}</h3><span className="text-emerald-600 font-bold">KES {parseFloat(room.price_per_night).toLocaleString()}<span className="text-stone-400 text-xs font-normal">/night</span></span></div><p className="text-stone-500 text-sm mb-3 line-clamp-2">{room.description||'No description'}</p><div className="flex items-center gap-1 text-stone-400 text-xs mb-3"><Users className="w-3 h-3" /> {room.max_guests} guests</div><div className="flex gap-3 pt-4 border-t"><button onClick={()=>openEdit(room)} className="flex-1 flex items-center justify-center gap-2 text-sm bg-amber-50 text-amber-700 py-2.5 rounded-xl hover:bg-amber-100"><Edit className="w-4 h-4" /> Edit</button><button onClick={()=>handleDelete(room.id)} className="flex-1 flex items-center justify-center gap-2 text-sm bg-red-50 text-red-500 py-2.5 rounded-xl hover:bg-red-100"><Trash2 className="w-4 h-4" /> Delete</button></div></div></div>
                ))}
              </div>
            ):(
              <div className="space-y-3">
                {paginatedRooms.map(room=>(
                  <div key={room.id} className="bg-white rounded-2xl border p-5 flex items-center justify-between hover:shadow-md transition"><div className="flex items-center gap-4"><div className="w-16 h-16 rounded-xl bg-stone-100 overflow-hidden flex-shrink-0">{room.image?<img src={room.image} className="w-full h-full object-cover" />:<div className="w-full h-full flex items-center justify-center"><BedDouble className="w-8 h-8 text-stone-300" /></div>}</div><div><h3 className="font-bold text-stone-800">Room {room.room_number} <span className="text-xs bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full capitalize ml-2">{room.room_type}</span></h3><p className="text-stone-500 text-sm">{room.description||'No description'}</p><div className="flex items-center gap-3 mt-1 text-xs text-stone-400"><Users className="w-3 h-3" /> {room.max_guests} guests</div></div></div><div className="flex items-center gap-6"><span className="font-bold text-stone-800">KES {parseFloat(room.price_per_night).toLocaleString()}<span className="text-stone-400 text-xs font-normal">/night</span></span><div className="flex gap-2"><button onClick={()=>openEdit(room)} className="px-4 py-2 text-sm bg-amber-50 text-amber-700 rounded-xl hover:bg-amber-100"><Edit className="w-4 h-4 inline mr-1" /> Edit</button><button onClick={()=>handleDelete(room.id)} className="px-4 py-2 text-sm bg-red-50 text-red-500 rounded-xl hover:bg-red-100"><Trash2 className="w-4 h-4 inline mr-1" /> Delete</button></div></div></div>
                ))}
              </div>
            )}
