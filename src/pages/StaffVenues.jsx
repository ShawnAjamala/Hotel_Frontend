...
import { Plus, Edit, Trash2, PartyPopper, X, ImagePlus, Users, Grid3X3, List, Package, PlusCircle, MinusCircle, AlertCircle } from 'lucide-react';
...
const StaffVenues = () => {
  ...
  // Popup state
  const [popup, setPopup] = useState({ show: false, type: '', message: '', onConfirm: null, venueName: '' });
  ...
  const showDeletePopup = (id, name) => {
    setPopup({
      show: true,
      type: 'danger',
      message: `Are you sure you want to deactivate "${name}"? This venue will no longer be available for booking.`,
  ...
  const handleDelete = async (id) => { 
    try { 
      ...
      setPopup({ show: false, type: '', message: '', onConfirm: null, venueName: '' });
      fetchVenues(); 
    } catch (err) {} 
  };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full mx-4 text-center animate-in fade-in zoom-in duration-200">
            ...
            <h3 className="text-xl font-bold text-stone-800 mb-2">
              {popup.type === 'danger' ? 'Deactivate Venue?' : 'Confirm Action'}
            </h3>
          ...
          <div className="bg-white rounded-2xl shadow-xl border overflow-hidden mb-10">
            <div className="bg-stone-50 px-6 py-4 border-b flex justify-between">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-lg font-bold">{editingVenue?'Edit Venue':'Create Venue'}</h2>
              </div>
            ...
            <form onSubmit={editingVenue?handleUpdate:handleCreate} className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <input type="text" value={form.name} onChange={e=>setForm({...form,name:e.target.value})} className="w-full px-3 py-2 border rounded-xl outline-none focus:ring-2 focus:ring-amber-500 text-sm" placeholder="Grand Ballroom" required />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-stone-700 mb-1">Venue Type *</label>
                    <select value={form.venue_type} onChange={e=>setForm({...form,venue_type:e.target.value})} className="w-full px-3 py-2 border rounded-xl outline-none focus:ring-2 focus:ring-amber-500 bg-white text-sm">
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <input type="number" value={form.capacity} onChange={e=>setForm({...form,capacity:e.target.value})} className="w-full px-3 py-2 border rounded-xl outline-none focus:ring-2 focus:ring-amber-500 text-sm" placeholder="200" required />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-stone-700 mb-1">Price/Day (KES) *</label>
                    <input type="number" value={form.price_per_day} onChange={e=>setForm({...form,price_per_day:e.target.value})} className="w-full px-3 py-2 border rounded-xl outline-none focus:ring-2 focus:ring-amber-500 text-sm" placeholder="50000" required />
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <input type="text" value={form.description} onChange={e=>setForm({...form,description:e.target.value})} className="w-full px-3 py-2 border rounded-xl outline-none focus:ring-2 focus:ring-amber-500 text-sm" placeholder="Elegant ballroom" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-stone-700 mb-1">Image</label>
                    <div className="border-2 border-dashed rounded-xl p-2 text-center hover:border-amber-400 cursor-pointer h-[80px] flex flex-col items-center justify-center bg-stone-50" onClick={()=>document.getElementById('vimg').click()}>
                      {imagePreview ? (
              ...
              <div className="mt-4 pt-3 border-t flex gap-3">
                <button type="submit" className="bg-amber-700 text-white px-6 py-2 rounded-xl font-medium hover:bg-amber-600 shadow-lg transition text-sm">{editingVenue?'Update':'Create'}</button>
